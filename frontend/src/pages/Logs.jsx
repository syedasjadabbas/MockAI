...
const formatLogItems = (data) => {
  if (!Array.isArray(data)) return [];
  return data.map(l => ({
    id: l._id ? l._id.slice(-6).toUpperCase() : 'LOG',
...
const Logs = () => {
  ...
  const hasCached = Array.isArray(cachedLogs) && cachedLogs.length > 0;

  const [logsData, setLogsData] = useState(() => hasCached ? formatLogItems(cachedLogs) : []);
  ...
  const fetchLogs = async (force = false) => {
    ...
    try {
      ...
      setLogsData(formatLogItems(data));
    } catch (err) {
      // ignore
    } finally {
      setLoading(false);
